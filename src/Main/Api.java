    ...
    for (int i = 0; i < producesMap.size(); i++) {
      ...
      if (!nameMap.get(i).isEmpty()) {
        for (int j = 0; j < nameMap.get(i).size(); j++) {
          ...
          String in;
          if (inMap.get(i).get(j).equalsIgnoreCase("query")) {
            in = "@QueryParam(\"" + nameMap.get(i).get(j) + "\") ";
          } else if (inMap.get(i).get(j).equalsIgnoreCase("path")) {
            in = "@PathParam(\"" + nameMap.get(i).get(j) + "\") ";
          } else {
            in = "    ";
          }
          auxConstructor +=
              in + typeMapping.get(typeMap.get(i).get(j)) + " " + nameMap.get(i).get(j) + ",";
        }
        auxReturn = auxReturn.substring(0, auxReturn.length() - 1);
        auxConstructor = auxConstructor.substring(0, auxConstructor.length() - 1);
      }

      if (consumesMap.get(i)) {
        consumes = "@Consumes(MediaType.APPLICATION_JSON)\n";
      }
      if (producesMap.get(i)) {
        produces = "@Produces(MediaType.APPLICATION_JSON)\n";
      }

      r +=
          "\n@"
              + pcoMap.get(i)[1].toUpperCase()
              + "\n"
              + "@Path(\""
              + pcoMap.get(i)[0]
              + "\")\n"
